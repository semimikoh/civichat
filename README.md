...
복지 검색은 조건 추출을 LLM 호출 없이 정규식으로 즉시 처리하고, 법령 검색은 짧은 후속 질문에서 직전 대화 문맥을 검색어에 안전하게 보강합니다. 검색 결과 요약에는 LLM SSE 스트리밍을 적용했고, 벡터 검색과 키워드 검색을 RRF로 결합한 하이브리드 검색 위에 조건/지역/키워드 기반 재정렬을 적용해 정확도를 높였습니다. 검색 결과 이벤트를 먼저 전송해 사용자가 응답 시작을 즉시 인지할 수 있도록 설계했습니다.
...
- 대화 히스토리 기반 멀티턴 검색
- 복지: 이전 조건 누적 / 법령: 짧은 후속 질문의 문맥 기반 검색어 보강
...
| 테스트     | Vitest + React Testing Library            | 유닛/컴포넌트 테스트 76개 + 검색 품질 평가       |
...
│   └── commands/        #   fetch, embed, search, legal-fetch, legal-embed, legal-search, eval
│
...
복지 검색은 나이/성별/직업/지역을 정규식 기반으로 즉시 추출합니다. LLM 호출 대비 지연 시간이 0에 가깝고, 대화 히스토리에서 이전 조건을 누적하여 멀티턴 검색을 지원합니다. 조건이 부족하면 추가 질문을 반환하고, `초기화`, `리셋`, `처음부터` 같은 명령어로 누적 조건을 비울 수 있습니다. 법령 검색은 복지처럼 조건을 구조화하지 않고, `비율은?`, `벌칙은?` 같은 짧은 후속 질문에서 최근 사용자 질문 1~2개를 붙여 검색어를 보강합니다. 정규식 패턴은 `extract-patterns.ts`에 분리해 새 조건을 추가하기 쉽게 구성했습니다.
...
### 법령 검색: 문맥 기반 질의 보강 + 하이브리드 검색 + LLM 요약

```
사용자 입력 ("장애인 고용 의무")
  |
  v
짧은 후속 질문이면 최근 사용자 질문 1~2개를 이어 붙여 검색어 보강
  -> "비율은?" => "장애인 고용 의무 비율은?"
  -> 구체적인 새 질문이면 이전 문맥 무시
  |
  v
...
대화 히스토리에서 이전에 언급한 **나이, 성별, 직업, 지역, 키워드**를 모두 누적하여 복지 멀티턴 검색을 지원합니다.
`취준생`, `소상공인`, `한부모`, `차상위`, `기초수급자`, `에너지 바우처` 같은 표현도 정규식 패턴으로 구조화합니다.

### 법령 멀티턴 상세

법령 검색은 조건 추출 대신 **문맥 기반 질의 보강**을 사용합니다. 사용자가 짧은 후속 질문을 입력하면 최근 사용자 질문 1~2개를 붙여 검색어를 구체화합니다.
...
"민간기업 기준으로"
"벌칙은?"
  -> "장애인 고용 의무 민간기업 기준으로 벌칙은?"
```

반대로 질문이 이미 충분히 구체적이거나 `처음부터`, `다른 주제` 같은 표현이 들어오면 이전 문맥을 잇지 않습니다.
...
| search/legal-context | 법령 후속 질문 문맥 보강, 새 주제 판별, 초기화 처리             | 5      |
